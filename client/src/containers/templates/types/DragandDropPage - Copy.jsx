		...
		fetch("http://buzzle.chrysalis.world/buzzleConceptCards-api/index.php/client/questionInfo?key=htn6ASfBr4X7GF29uCciOPeDaysWjJMx",{
			...
			.then(json=>{
				if(json) {					
					...
					textFields['answertext'] = json['que_info']['drop']['text']
	...
	*/
    addOption () {
        ...
        let optionvalues = {
			...
			feedbackimage:optionFiles['feedbackimage']
    ...
    */
    onRowClickEdit(index){
        return function(event){
			...
			textFields['feedbackimage'] = questionContent['option'][index]['feedbackimage']
    ...
    */
    onSubmit(event) {
		...
		if(validationstatus){
			const textFields = this.state.textFields			
			const questionContent = this.state.questionContent
			questionContent['drop']['text'] = textFields['answertext'];
