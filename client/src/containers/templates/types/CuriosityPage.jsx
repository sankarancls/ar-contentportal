...
import CuriosityForm from '../../../components/templates/types/CuriosityForm.jsx';
...
const required_fields = ['questiontext', 'answertext']
let activity_shortcode = '0';
class CuriosityPage extends React.Component {
    /**
    * Class Constructor
    */    
    constructor(props){
		...
		fetch("http://buzzle.chrysalis.world/buzzleConceptCards-api/index.php/client/questionInfo?key=htn6ASfBr4X7GF29uCciOPeDaysWjJMx",{
			...
			.then(json=>{
				if(json) {					
					...
					textFields['answertext'] = json['que_info']['iknow']['text']					
    ...
    */
    onSubmit(event) {
		...
		if(validationstatus){
			...
			questionContent['iknow']['text'] = textFields['answertext'];
			this.setState({
				questionContent:questionContent
			}, function(){				
				this.update_ajax()
