...
import DTORoboForm from '../../../components/templates/types/DTORoboForm.jsx';
...
const required_fields = ['instruction','questiontext', 'answertext','dropcategory','droplegends']
let activity_shortcode = '0';
class DTORoboPage extends React.Component {
    /**
    * Class Constructor
    */    
    constructor(props){
		...
		fetch("http://buzzle.chrysalis.world/buzzleConceptCards-api/index.php/client/questionInfo?key=htn6ASfBr4X7GF29uCciOPeDaysWjJMx",{
			...
			.then(json=>{
				if(json) {					
					...
					textFields['instructiontitle'] = json['que_info']['instruction']['title']
					...
					textFields['dropcategory'] = json['que_info']['answer']['dropcategory']
					textFields['droplegends'] = json['que_info']['answer']['droplegends']					
    ...
    */
    onSubmit(event) {
		...
		if(validationstatus){
			...
			questionContent['instruction']['title'] = textFields['instructiontitle'];
			...
			questionContent['answer']['dropcategory'] = textFields['dropcategory'];
			questionContent['answer']['droplegends'] = textFields['droplegends'];
			this.setState({
				questionContent:questionContent
			}, function(){				
				this.update_ajax()
	...
	*/
	clearFields() {
		...
		textFields['dropcategory'] = '';
		textFields['droplegends'] = '';
