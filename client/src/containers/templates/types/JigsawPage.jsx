...
import JigsawForm from '../../../components/templates/types/JigsawForm.jsx';
...
const required_fields = ['rowcount','columncount']
let activity_shortcode = '0';
class JigsawPage extends React.Component {
    /**
    * Class Constructor
    */    
    constructor(props){
		...
		fetch("http://buzzle.chrysalis.world/buzzleConceptCards-api/index.php/client/questionInfo?key=htn6ASfBr4X7GF29uCciOPeDaysWjJMx",{
			...
			.then(json=>{
				if(json) {					
					...
					textFields['rowcount'] = json['que_info']['rowcount']
					textFields['columncount'] = json['que_info']['columncount']					
    ...
    */
    loadPictures(event) {
        ...
        for(var i=0;i<event.target.files.length;i++){
            ...
            questionContent['jigsaw'].push(addnewImage);
        }
    ...
    */
    onSubmit(event) {
		...
		if(validationstatus){
			const textFields = this.state.textFields			
			const questionContent = this.state.questionContent
			questionContent['rowcount'] = textFields['rowcount'];
			questionContent['columncount'] = textFields['columncount'];			
    ...
    */
    onDeletePictureIndex(index,imagename) {
      return function(event) {
          const questionContent = this.state.questionContent
          questionContent['jigsaw'].splice(index,1);          
